      ...
      body: FutureBuilder<GetEvolutionOut>(
        ...
        builder: (context, snapshot) {
          ...
          return snapshot.error != null
              ...
              : data.has
              ? _CeinturesEvolution(widget.api, widget.tokens, data.evolution)
        ...
        child: Column(
          ...
          children: [
            ...
            pending == null
                ...
                : CeintureIcon(pending, scale: 1.3, withBackground: true),
          ],
        ),
      ),
    );
...
class _StageDetailsDialog extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final needed = scheme.ps
        .where(
          (s) => s.pending.domain == stage.domain && s.pending.rank == pending,
        )
        .map((p) => p.need)
        .toList();
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: pending == null
            ? const [Text("Domaine terminé. Bravo !")]
            : locked
            ? [
                const Text(
            ...
            : [
                ...
                const SizedBox(height: 12),
                CeintureIcon(pending, withBackground: true),
                const SizedBox(height: 16),
                if (pendingStat != null)
                  Text(
