...
class ProofFieldBlock implements Block {
  final Proof shape;
  final List<String> termProposals;
  final int iD;
...
ProofFieldBlock proofFieldBlockFromJson(dynamic json_) {
  ...
  return ProofFieldBlock(proofFromJson(json['Shape']),
      listStringFromJson(json['TermProposals']), intFromJson(json['ID']));
}

JSON proofFieldBlockToJson(ProofFieldBlock item) {
  return {
    "Shape": proofToJson(item.shape),
    "TermProposals": listStringToJson(item.termProposals),
    "ID": intToJson(item.iD)
  };
}
