...
func randAdvance() Advance {
	return Advance(randAr12_Rank())
}

func randAr11_Stat() [11]Stat {
	var out [11]Stat
	for i := range out {
		out[i] = randStat()
	}
...
func randAr12_Ar11_Stat() [12][11]Stat {
	var out [12][11]Stat
	for i := range out {
		out[i] = randAr11_Stat()
	}
...
func randDomain() Domain {
	choix := [...]Domain{CalculMentalI, CalculMentalII, Puissances, Fractions, Reduction, Factorisation, Developpement, IsolerVariable, Equations, Inequations, Derivation, Matrices}
...
func randLevel() Level {
	choix := [...]Level{Seconde, Premiere, Terminale, PostBac}
	i := rand.Intn(len(choix))
	return choix[i]
}
...
func randStats() Stats {
	return Stats(randAr12_Ar11_Stat())
}
