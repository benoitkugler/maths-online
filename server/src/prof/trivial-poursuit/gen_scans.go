...
func (item TrivialConfig) Insert(tx DB) (out TrivialConfig, err error) {
	row := tx.QueryRow(`INSERT INTO trivial_configs (
		IsLaunched,Questions,QuestionTimeout,GroupStrategy,MaxPlayersPerGroup
		) VALUES (
		$1,$2,$3,$4,$5
		) RETURNING 
		Id,IsLaunched,Questions,QuestionTimeout,GroupStrategy,MaxPlayersPerGroup;
		`, item.IsLaunched, item.Questions, item.QuestionTimeout, item.GroupStrategy, item.MaxPlayersPerGroup)
	return ScanTrivialConfig(row)
}
...
func (item TrivialConfig) Update(tx DB) (out TrivialConfig, err error) {
	row := tx.QueryRow(`UPDATE trivial_configs SET (
		...
		Id,IsLaunched,Questions,QuestionTimeout,GroupStrategy,MaxPlayersPerGroup;
		`, item.Id, item.IsLaunched, item.Questions, item.QuestionTimeout, item.GroupStrategy, item.MaxPlayersPerGroup)
	return ScanTrivialConfig(row)
}
